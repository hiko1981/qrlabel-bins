...
export async function POST(req: Request) {
  ...
  const challenge = jar.get('webauthn_admin_login_challenge')?.value;
  jar.set('webauthn_admin_login_challenge', '', { path: '/', maxAge: 0 });
  ...
  const isAdmin = await isAdminUser(cred.user_id);
  ...
  await applySessionToResponse(res, cred.user_id, req);
  return res;
}
