...
export default async function OwnerLocationSelectPage() {
  ...
  if (match?.locatorToken) redirect(`/owner/location/${encodeURIComponent(match.locatorToken)}`);
  if (bins.length === 1 && bins[0]?.locatorToken) redirect(`/owner/location/${encodeURIComponent(bins[0].locatorToken)}`);
        ...
        {bins.map((b) =>
          b.locatorToken ? (
            <Link
              ...
              href={`/owner/location/${encodeURIComponent(b.locatorToken)}`}
            >
              {b.label}
            </Link>
          ) : null,
